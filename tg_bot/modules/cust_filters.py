...
def stop_filter(bot: Bot, update: Update):
    ...
    update.effective_message.reply_text("That's not a current filter - run /list for all active filters.")
                ...
                try:
                    ...
                    if excp.message == "Unsupported url protocol":
                        message.reply_text("You seem to be trying to use an unsupported url protocol. Telegram "
                                           ...
                                           "again, or ask in @Mo_Tech_Group for help.")
                        ...
                        message.reply_text("This note could not be sent, as it is incorrectly formatted. Ask in "
                                           "@Mo_Tech_Group if you can't figure out why!")
...
*Admin Only*
 ...
 - /deall - Stop all filters.
   *Chat creator only:*

 - /list - List all filters in chat
"""
...
RMALLFILTER_HANDLER = CommandHandler("delall", rmall_filters, filters=Filters.group)
RMALLFILTER_CALLBACK = CallbackQueryHandler(rmall_callback, pattern=r"filters_.*")
LIST_HANDLER = DisableAbleCommandHandler("list", list_handlers, admin_ok=True)
